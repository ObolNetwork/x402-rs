...
use x402_chain_eip155::{
    V1Eip155Exact, V2Eip155AuthCapture, V2Eip155BatchSettlement, V2Eip155Exact, V2Eip155Upto,
};
...
#[cfg(feature = "chain-eip155")]
impl X402SchemeFacilitatorBuilder<&ChainProvider> for V2Eip155AuthCapture {
    fn build(
        &self,
        provider: &ChainProvider,
        config: Option<serde_json::Value>,
        ...
        let eip155_provider = if let ChainProvider::Eip155(provider) = provider {
            ...
            return Err(
                "V2Eip155AuthCapture::build: provider must be an Eip155ChainProvider".into(),
            );
        };
        self.build(eip155_provider, config)
    }
